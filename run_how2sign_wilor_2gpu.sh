...
LOG_DIR="${REPO_DIR}/logs/how2sign_wilor"
...
start_worker() {
    ...
    mkdir -p \
        "/tmp/wilor_matplotlib_${shard_index}" \
        "/tmp/wilor_ultralytics_${shard_index}"

    nohup env \
        CUDA_VISIBLE_DEVICES="${gpu_id}" \
        MPLCONFIGDIR="/tmp/wilor_matplotlib_${shard_index}" \
        YOLO_CONFIG_DIR="/tmp/wilor_ultralytics_${shard_index}" \
        PYTHONUNBUFFERED=1 \
